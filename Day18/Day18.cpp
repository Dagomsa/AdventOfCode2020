...
int CalculateDeepMapParentheses(const std::string& str, std::string& strmap);
int CalculateDeepMapSum(const std::string& str, std::string& strmap);
...
{
    std::string strmap;
    int maxdeep = CalculateDeepMapParentheses(str, strmap);
    ...
    while (maxdeep >= 0)
    {
        ...
        if (maxdeep != 0)
        {
            ...
            maxdeep = CalculateDeepMapParentheses(str, strmap);
        }
        else
        {
            operate(str);
...
uint64_t Solve2(std::string& str)
{
    ...
    while (maxdeep >= 0)
    {
        ...
        if (maxdeep != 0)
        {
            ...
            std::string strmap2;
            int maxdeepsum = CalculateDeepMapSum(deepop, strmap2);
            while (maxdeepsum > 0)
            {
                ...
                maxdeepsum = CalculateDeepMapSum(deepop, strmap2);
            }
        ...
        {
            std::string strmap2;
            int maxdeepsum = CalculateDeepMapSum(str, strmap2);
            while (maxdeepsum > 0)
            {
                std::string level = std::to_string(maxdeepsum);
                int pos1s = strmap2.find(level);
                ...
                if (pos2s == strmap2.npos)
                    pos2s = strmap2.length();
                std::string deepopsum = str.substr(pos1s, pos2s - pos1s);
                operatesum(deepopsum);
                str.replace(pos1s, pos2s - pos1s, deepopsum);
                maxdeepsum = CalculateDeepMapSum(str, strmap2);
            }
            operate(str);
            return stoull(str);
        }
